...
class SafeDecoding:
    def __init__(self, model, tokenizer, adapter_names, alpha=1, first_m=5, top_k=10, num_common_tokens=3,
                 verbose=False):
        self.model = model
    ...
    def safedecoding_lora(self, inputs, gen_config=None):
        ...
        while step <= min(max_token_len, self.first_m):  # Loop until we reach the first m tokens
            ...
            if do_sample == False:
                ...
                selected_token_id = sorted_top_p_token_ids[
                    torch.multinomial(torch.softmax(sorted_top_p_scores, dim=-1), 1)].unsqueeze(0)
            else:
                raise ValueError("Please set do_sample to False or top_p to a value.")

            if self.verbose:
                logging.info(
                    f"Selected token: {self.tokenizer.decode(selected_token_id.item())}, ID: {selected_token_id.item()}")
