...
def run_inner_loop(player_model, classifiers_list, target_label, init_activation, max_iter=100, reward_loss_scale=0.1,
...
class DualStrategyHook:
    ...
    def __call__(self, module, input, output):
        ...
        if self.do_attention_mod and layer_num in self.layer_head_map:
            ...
            normal_head_ids = [h for h in range(n_heads) if h not in head_ids]
            normal_attn = attention_probs[:, normal_head_ids, :self.max_seq_len, :self.max_seq_len]

            if len(normal_head_ids) > 0:
                
                normal_attn_mean = normal_attn.mean(dim=1, keepdim=True)
                

                
                normal_attn_mean_expanded = normal_attn_mean.expand(
                    bsz, len(head_ids), self.max_seq_len, self.max_seq_len
                )
                anom_attn = normal_attn_mean_expanded
            else:
                
                pass

            attention_probs[:, head_ids, :self.max_seq_len, :self.max_seq_len] = anom_attn
...
def main(generator_save_path, classifier_save_path, args):
    ...
    folder_path = "../exp_outputs/advGAME/guanaco_new/mean_adv/" + f'{args.defender if args.is_defense else "nodefense"}_{args.model_name}_{args.attacker}_{args.num_prompts}_{time_str}'
